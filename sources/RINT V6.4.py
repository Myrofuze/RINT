...
"""
import ctypes
ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID("RINT.IDE")
...
class Minimap(tk.Canvas):
    def __init__(self, parent, text_widget):
        super().__init__(parent, bg=t("bg"), highlightthickness=0, width=90, cursor="hand2")
        self.text = text_widget
    ...
    def schedule_redraw(self, *_):
        ...
        self._after_id = self.after(250, self.redraw)
    ...
    def redraw(self):
        ...
        W = 90
        H = max(1, self.winfo_height()) or 600
        total = max(1, len(lines))
        # Dynamic scale: fit all lines in canvas height, min 2px/line max 4px
        scale_y = max(2.0, min(4.0, H / total))

        for li, line in enumerate(lines[:500]):
            y = li * scale_y
            if y > H: break
            stripped = line.strip()
            if not stripped:
            ...
            if stripped.startswith("//"):
                color = t("token_cm")
            elif re.match(r'\s*(func|class)\s', line):
                color = t("accent")
            elif re.match(r'\s*(if|elif|else|while|when|repeat)\b', line):
                color = t("token_kw")
            elif re.match(r'\s*(int|string|bool|float|auto|const)\s', line):
                color = t("token_type")
                ...
                color = t("fg3")
            x0 = max(2, indent // 3 + 2)
            x1 = min(W - 2, x0 + max(6, len(stripped) // 2))
            h  = max(1, int(scale_y) - 1)
            self.create_rectangle(x0, y, x1, y + h, fill=color, outline="")

        # Viewport indicator — visible solid rectangle
        try:
            first = int(self.text.index("@0,0").split(".")[0])
            last  = int(self.text.index(f"@0,{self.winfo_height()}").split(".")[0])
            y1 = first * scale_y
            y2 = max(y1 + 6, last * scale_y)
            # Semi-transparent fill with accent border
            self.create_rectangle(0, y1, W, y2,
                                  outline=t("accent"), fill=t("bg4"),
                                  stipple="gray50", width=1)
            self.create_line(0, y1, W, y1, fill=t("accent"), width=1)
            self.create_line(0, y2, W, y2, fill=t("accent"), width=1)
        except:
            pass
    ...
    def _jump(self, event):
        ...
        target = max(1, int(frac * total_lines))
...
class SnippetManager(tk.Toplevel):
    def __init__(self, parent, ide):
        super().__init__(parent); self.ide = ide
        self.title("Snippet Manager"); self.configure(bg=t("bg")); self.geometry("700x480")
        tk.Frame(self, bg=t("accent"), height=3).pack(fill="x")
        tk.Label(self, text="📋  Snippet Manager", bg=t("bg"), fg=t("accent"),
                 font=("JetBrains Mono", 12, "bold")).pack(pady=(12, 4))
        main = tk.Frame(self, bg=t("bg")); main.pack(fill="both", expand=True, padx=12, pady=4)
        # Left list
        lf = tk.Frame(main, bg=t("bg1"), width=180); lf.pack(side="left", fill="y", padx=(0, 8)); lf.pack_propagate(False)
        tk.Label(lf, text="SNIPPETS", bg=t("bg1"), fg=t("fg3"), font=F_UIS).pack(fill="x", padx=8, pady=4)
        self.snip_lb = tk.Listbox(lf, bg=t("bg1"), fg=t("fg"), selectbackground=t("sel"),
                                   font=F_UIS, borderwidth=0, highlightthickness=0)
        self.snip_lb.pack(fill="both", expand=True, padx=4, pady=4)
        br = tk.Frame(lf, bg=t("bg1")); br.pack(fill="x", padx=4, pady=4)
        for txt, cmd in [("+ New", self._new_snip), ("Delete", self._del_snip)]:
            tk.Button(br, text=txt, command=cmd, bg=t("bg3"), fg=t("fg"),
                      relief="flat", font=F_UIS, padx=8).pack(side="left", padx=2)
        # Right editor
        rf = tk.Frame(main, bg=t("bg")); rf.pack(side="left", fill="both", expand=True)
        r1 = tk.Frame(rf, bg=t("bg")); r1.pack(fill="x", pady=(0, 4))
        tk.Label(r1, text="Name:", bg=t("bg"), fg=t("fg2"), font=F_UIS).pack(side="left")
        self.name_v = tk.StringVar()
        tk.Entry(r1, textvariable=self.name_v, bg=t("bg3"), fg=t("fg"), font=F_CODE,
                 insertbackground=t("accent"), relief="flat", width=22).pack(side="left", padx=8)
        self.snip_text = tk.Text(rf, bg=t("bg2"), fg=t("fg"), font=F_CODE,
                                  borderwidth=0, highlightthickness=1,
                                  highlightbackground=t("border"), insertbackground=t("accent"), height=14)
        self.snip_text.pack(fill="both", expand=True)
        # Buttons
        br2 = tk.Frame(self, bg=t("bg")); br2.pack(fill="x", padx=12, pady=8)
        tk.Button(br2, text="Insert into Editor", command=self._insert,
                  bg=t("accent"), fg=t("bg"), relief="flat", font=F_UIB, padx=16, pady=4).pack(side="left", padx=4)
        tk.Button(br2, text="Save Snippet", command=self._save_snip,
                  bg=t("bg3"), fg=t("fg"), relief="flat", font=F_UIS, padx=12).pack(side="left", padx=4)
        tk.Button(br2, text="Close", command=self.destroy,
                  bg=t("bg3"), fg=t("fg"), relief="flat", font=F_UIS, padx=12).pack(side="right", padx=4)
        self._refresh_list()
        self.snip_lb.bind("<<ListboxSelect>>", self._load_snip)

    def _refresh_list(self):
        self.snip_lb.delete(0, "end")
        for k in SNIPPETS: self.snip_lb.insert("end", f"  {k}")

    def _load_snip(self, *_):
        sel = self.snip_lb.curselection()
        if not sel: return
        key = self.snip_lb.get(sel[0]).strip()
        self.name_v.set(key)
        self.snip_text.delete("1.0", "end"); self.snip_text.insert("1.0", SNIPPETS.get(key, ""))

    def _new_snip(self): self.name_v.set("new_snippet"); self.snip_text.delete("1.0", "end")

    def _del_snip(self):
        sel = self.snip_lb.curselection()
        if not sel: return
        key = self.snip_lb.get(sel[0]).strip()
        if key in SNIPPETS: del SNIPPETS[key]; self._refresh_list()

    def _save_snip(self):
        key = self.name_v.get().strip()
        if not key: return
        SNIPPETS[key] = self.snip_text.get("1.0", "end-1c"); self._refresh_list()

    def _insert(self):
        key = self.name_v.get().strip()
        content = SNIPPETS.get(key, self.snip_text.get("1.0", "end-1c"))
        self.ide._insert_snippet_text(content); self.destroy()


class WorkspaceSettings(tk.Toplevel):
    def __init__(self, parent, ide):
        super().__init__(parent); self.ide = ide
        self.title("Workspace Settings"); self.configure(bg=t("bg")); self.geometry("480x400"); self.resizable(False, False)
        tk.Frame(self, bg=t("accent"), height=3).pack(fill="x")
        tk.Label(self, text="⚙  Workspace Settings", bg=t("bg"), fg=t("accent"),
                 font=("JetBrains Mono", 12, "bold")).pack(pady=(12, 4))
        if not hasattr(ide, "ws_config"):
            ide.ws_config = {}
        cfg = ide.ws_config
        f = tk.Frame(self, bg=t("bg")); f.pack(fill="both", expand=True, padx=20, pady=8)
        self.vars_ = {}
        settings = [
            ("tab_size",   "Tab Size (spaces)",          "4"),
            ("font_size",  "Font Size",                  "11"),
            ("theme",      "Default Theme",              "noir"),
            ("auto_save",  "Auto-save interval (sec, 0=off)", "0"),
            ("word_wrap",  "Word Wrap (1/0)",             "0"),
            ("linting",    "Live Linting (1/0)",          "1"),
        ]
        for key, label, default in settings:
            row = tk.Frame(f, bg=t("bg")); row.pack(fill="x", pady=4)
            tk.Label(row, text=label + ":", bg=t("bg"), fg=t("fg2"), font=F_UIS, width=30, anchor="w").pack(side="left")
            v = tk.StringVar(value=str(cfg.get(key, default)))
            tk.Entry(row, textvariable=v, bg=t("bg3"), fg=t("fg"), font=F_CODE,
                     insertbackground=t("accent"), relief="flat", width=14).pack(side="left", padx=8)
            self.vars_[key] = v
        br = tk.Frame(self, bg=t("bg")); br.pack(fill="x", padx=20, pady=12)
        tk.Button(br, text="Apply", command=self._apply, bg=t("accent"), fg=t("bg"),
                  relief="flat", font=F_UIB, padx=16, pady=4).pack(side="left", padx=4)
        tk.Button(br, text="Cancel", command=self.destroy, bg=t("bg3"), fg=t("fg"),
                  relief="flat", font=F_UIS, padx=12).pack(side="right", padx=4)

    def _apply(self):
        for k, v in self.vars_.items(): self.ide.ws_config[k] = v.get()
        self._apply_ws(); self.destroy()

    def _apply_ws(self):
        cfg = self.ide.ws_config
        try:
            fs = int(cfg.get("font_size", "11"))
            global F_CODE, F_CODES
            F_CODE = ("JetBrains Mono", fs); F_CODES = ("JetBrains Mono", max(8, fs - 2))
            for tab in self.ide.tabs: tab["text"].configure(font=F_CODE)
        except: pass
        theme = cfg.get("theme")
        if theme and theme in THEMES: self.ide._set_theme(theme)


class TaskRunnerPanel(tk.Toplevel):
    DEFAULT_TASKS = [
        ("▶ Run RINT file",   "python {file}"),
        ("🧹 Clean .pyc",     "python -c \"import pathlib; [f.unlink() for f in pathlib.Path('.').rglob('*.pyc')]\""),
        ("📦 Install deps",   "pip install customtkinter psutil"),
    ...
    def __init__(self, parent, ide):
        super().__init__(parent); self.ide = ide
        self.title("Task Runner"); self.configure(bg=t("bg")); self.geometry("580x420")
        tk.Frame(self, bg=t("accent"), height=3).pack(fill="x")
        tk.Label(self, text="⚡  Task Runner", bg=t("bg"), fg=t("accent"),
                 font=("JetBrains Mono", 12, "bold")).pack(pady=(12, 4))
        main = tk.Frame(self, bg=t("bg")); main.pack(fill="both", expand=True, padx=12, pady=4)
        self.cmd_vars = []
        for name, cmd in self.DEFAULT_TASKS:
            row = tk.Frame(main, bg=t("bg2")); row.pack(fill="x", pady=2)
            tk.Label(row, text=name, bg=t("bg2"), fg=t("fg"), font=F_UIB, width=22, anchor="w").pack(side="left", padx=12, pady=5)
            cv = tk.StringVar(value=cmd)
            self.cmd_vars.append(cv)
            tk.Entry(row, textvariable=cv, bg=t("bg3"), fg=t("fg2"), font=F_UIS,
                     insertbackground=t("accent"), relief="flat", width=26).pack(side="left", padx=4)
            tk.Button(row, text="▶", bg=t("accent"), fg=t("bg"), relief="flat", font=F_UIB,
                      command=lambda c=cv: self._run_task(c.get()),
                      padx=6, pady=2).pack(side="right", padx=8)
        tk.Label(self, text="Output:", bg=t("bg"), fg=t("fg3"), font=F_UIS).pack(anchor="w", padx=12)
        self.out = tk.Text(self, bg=t("bg2"), fg=t("green"), font=F_CODES,
                            borderwidth=0, highlightthickness=0, height=7)
        self.out.pack(fill="x", padx=12, pady=4)
        tk.Button(self, text="Close", command=self.destroy,
                  bg=t("bg3"), fg=t("fg"), relief="flat", font=F_UIS, padx=12).pack(pady=6)

    def _run_task(self, cmd):
        tab = self.ide._active_tab_data()
        if tab and "{file}" in cmd:
            cmd = cmd.replace("{file}", tab.get("path") or "untitled.rint")
        self.out.delete("1.0", "end"); self.out.insert("end", f"$ {cmd}\n")
        def run():
            try:
                r = subprocess.run(cmd, shell=True, capture_output=True, text=True,
                                   cwd=self.ide.term_cwd, timeout=30, encoding="utf-8", errors="replace")
                output = (r.stdout + r.stderr) or "(no output)\n"
            except subprocess.TimeoutExpired: output = "Timeout (30s)\n"
            except Exception as e: output = f"Error: {e}\n"
            self.out.after(0, lambda: self.out.insert("end", output))
...
class RintStudio(ctk.CTk):
    ...
    def _build_layout(self):
        ...
        self._build_sidebar()
        self._build_right_panel()   # pack right BEFORE center so it anchors right
        self._build_center()
    ...
    def _build_toolbar(self):
        ...
        # Panel hide/show buttons — right side of toolbar
        tk.Frame(tb, bg=t("border"), width=1).pack(side="right", fill="y", pady=8, padx=4)
        panel_btns = [
            ("◨", self._toggle_right,   "Toggle Right Panel"),
            ("▤", self._toggle_bottom,  "Toggle Console  "),
            ("◧", self._toggle_sidebar, "Toggle Sidebar  Ctrl+B"),
        ]
        for icon, cmd, tip in panel_btns:
            btn = tk.Button(tb, text=icon, command=cmd,
                            bg=t("bg1"), fg=t("fg3"), relief="flat",
                            font=("Segoe UI", 12), padx=7, pady=4,
                            activebackground=t("bg4"), activeforeground=t("accent"),
                            cursor="hand2")
    ...
    def _build_center(self):
        ...
        # *** FIX: build bottom FIRST so it reserves space at the bottom ***
        self._build_bottom()

        # Editor area — packed AFTER bottom so it fills remaining space
    ...
    def new_tab(self, content="", filename=None):
        ...
        txt_x  = tk.Scrollbar(ef, orient="horizontal", command=txt.xview,
                               bg=t("bg"), troughcolor=t("bg"), width=8)
        ...
        def on_key(_=None, i=idx):
            ...
            # Live outline refresh — debounced 400ms
            if hasattr(self, "_outline_after"):
                self.after_cancel(self._outline_after)
            self._outline_after = self.after(400, self._refresh_outline)
    ...
    def _insert_snippet_text(self, content):
        tw = self._active_text()
        if tw: tw.insert("insert", content)
    ...
    def _toggle_bottom(self):
        if not hasattr(self, "bottom_vis"): self.bottom_vis = True
        if self.bottom_vis:
            self.bottom.pack_forget()
            self.bottom_vis = False
        else:
            # Must repack before editor_area
            self.editor_area.pack_forget()
            self.bottom.pack(fill="x", side="bottom")
            self.editor_area.pack(fill="both", expand=True)
            self.bottom_vis = True
    ...
    def _init_turtle(self):
        ...
        if w < 10: w = 800
        if h < 10: h = 160
        # Grille (10 pixels = 1 case)
        for x in range(0, w, 10):
            c.create_line(x, 0, x, h, fill=t("bg3"), dash=(2,2), width=1)
        for y in range(0, h, 10):
            c.create_line(0, y, w, y, fill=t("bg3"), dash=(2,2), width=1)
    ...
    def _draw_turtle(self):
        """Dessine la tortue comme un triangle"""
        c = self.turtle_canvas
        # Effacer l'ancienne tortue
        c.delete("turtle")
        import math as m_
        size = 12
        angle_rad = m_.radians(self.turtle_angle)
        # Triangle pointant dans la direction
        p1_x = self.turtle_x + size * m_.cos(angle_rad)
        p1_y = self.turtle_y - size * m_.sin(angle_rad)
        p2_x = self.turtle_x + size * m_.cos(angle_rad + 2.4) 
        p2_y = self.turtle_y - size * m_.sin(angle_rad + 2.4)
        p3_x = self.turtle_x + size * m_.cos(angle_rad - 2.4)
        p3_y = self.turtle_y - size * m_.sin(angle_rad - 2.4)
        c.create_polygon(p1_x, p1_y, p2_x, p2_y, p3_x, p3_y,
                        fill=t("accent"), outline=t("accent2"), width=2, tags="turtle")
        c.create_oval(self.turtle_x-3, self.turtle_y-3, 
                     self.turtle_x+3, self.turtle_y+3,
                     fill=t("accent2"), outline="", tags="turtle")

    def _turtle_cmd(self, cmd, args):
        c = self.turtle_canvas
        try:
            if cmd in ["forward", "move"]:
...
<pre>"""
    ...
    def _toggle_sidebar(self):
        if self.sidebar_vis:
            ...
            self.act.pack(side="left", fill="y", before=self.center)
            self.sidebar.pack(side="left", fill="y", before=self.center)
            self.sidebar_vis = True

    def _toggle_right(self):
        if not hasattr(self, "right_vis"): self.right_vis = True
        if self.right_vis:
            self.right.pack_forget()
            self.right_vis = False
        else:
            self.right.pack(side="right", fill="y", before=self.center)
            self.right_vis = True
    ...
    def _set_theme(self, key):
        global _T
        old_theme = THEMES[_T]
        # Build reverse map: hex_color -> semantic_key for old theme
        old_rev = {v: k for k, v in old_theme.items() if isinstance(v, str) and v.startswith("#")}
        _T = key
        new_theme = THEMES[_T]
        # Walk ALL widgets and swap colors
        self._theme_walk(self, old_rev, new_theme)
        # Re-apply syntax highlighting tags for all open tabs
        for tab in self.tabs:
            if tab["highlighter"]:
                tab["highlighter"].refresh_tags()
                tab["highlighter"].highlight()
            tab["linenums"].configure(bg=t("gutter"))
            tab["linenums"].redraw()
            tab["minimap"].configure(bg=t("bg"))
            tab["minimap"].schedule_redraw()
        # Status bar background
        self.status_bar.configure(bg=t("status"))
        self.st_left.configure(bg=t("status"))
        self.st_right.configure(bg=t("status"))
        self.st_center.configure(bg=t("status"))
        # Theme label
        self._theme_label.configure(text=f"⬤  {new_theme['name']}", fg=t("accent"), bg=t("bg1"))
        self.st_left.configure(text=f"🎨 Theme: {new_theme['name']}")

    def _theme_walk(self, widget, old_rev, new_theme):
        """Recursively walk widget tree and update colors"""
        wclass = widget.winfo_class()
        color_attrs = {
            "Frame":       ["bg"],
            "Label":       ["bg", "fg"],
            "Button":      ["bg", "fg", "activebackground", "activeforeground"],
            "Entry":       ["bg", "fg", "insertbackground", "highlightbackground", "highlightcolor"],
            "Text":        ["bg", "fg", "insertbackground", "selectbackground"],
            "Listbox":     ["bg", "fg", "selectbackground", "selectforeground"],
            "Canvas":      ["bg"],
            "Checkbutton": ["bg", "fg", "selectcolor", "activebackground"],
            "Menu":        ["bg", "fg", "activebackground", "activeforeground"],
            "Scrollbar":   ["bg", "troughcolor"],
        }
        attrs = color_attrs.get(wclass, [])
        if attrs:
            updates = {}
            for attr in attrs:
                try:
                    ...
                    if cur in old_rev:
                        new_val = new_theme.get(old_rev[cur])
                        if new_val:
        ...
        try:
            for child in widget.winfo_children():
                self._theme_walk(child, old_rev, new_theme)
        except Exception:
            pass
    ...
    def _exec_command(self, cmd):
        dispatch = {
            ...
            "spotlight":     lambda: (self._switch_sidebar("search"), self._spotlight_do()),
            "goto":          self._goto_line,
            "goto_def":      self._goto_definition,
            "find_refs":     self._find_references,
            "bookmark":      self._toggle_bookmark,
            "next_bookmark": self._next_bookmark,
            "rename":        self._rename_symbol,
            "quickfix":      self._quick_fix,
            ...
            "right":         self._toggle_right,
            "console":       self._toggle_bottom,
            "format":        self._format_code,
            "gen_doc":       self._gen_doc,
            "smart_import":  self._smart_import,
            "snippets":      lambda: SnippetManager(self, self),
            "colorpick":     self._color_picker,
            "split":         self._toggle_split,
            "history":       self._show_history,
            "save_session":  self._save_session,
            "load_session":  self._load_session,
            "task_runner":   lambda: TaskRunnerPanel(self, self),
            "ws_settings":   lambda: WorkspaceSettings(self, self),
            "terminal":      lambda: self._switch_console("Terminal"),
        ...
        if fn:
            try: fn()
            except Exception as e: self._console_err(f"Command error '{cmd}': {e}")
    ...
    def _goto_definition(self):
        ...
        col = int(pos.split(".")[1])
        s = col
        while s > 0 and (line_content[s-1].isalnum() or line_content[s-1] == "_"): s -= 1
        e = col
        while e < len(line_content) and (line_content[e].isalnum() or line_content[e] == "_"): e += 1
        symbol = line_content[s:e]
        if not symbol: return
        code = tw.get("1.0", "end-1c")
        for i, line in enumerate(code.split("\n"), 1):
            stripped = line.strip()
            if (re.match(rf'^func\s+{re.escape(symbol)}\s*\(', stripped) or
                re.match(rf'^class\s+{re.escape(symbol)}\b', stripped) or
                re.match(rf'^\w+\s+{re.escape(symbol)}\s*=', stripped)):
                tw.see(f"{i}.0"); tw.mark_set("insert", f"{i}.0")
                tab["highlighter"]._highlight_current_line()
                self.st_left.configure(text=f"→ Definition of '{symbol}' at line {i}")
                return
        self.st_left.configure(text=f"❌ '{symbol}' not found in this file")
    ...
    def _find_references(self):
        ...
        code = tw.get("1.0", "end-1c")
        self.search_results.delete(0, "end")
        self.search_results.insert("end", f"Refs to '{symbol}':")
        count = 0
        for i, line in enumerate(code.split("\n"), 1):
            if re.search(r'\b' + re.escape(symbol) + r'\b', line):
                self.search_results.insert("end", f"  [current]:{i}  {line.strip()[:55]}")
                count += 1
        self._switch_sidebar("search")
        self.st_left.configure(text=f"🔗 {count} reference(s) to '{symbol}'")
    ...
    def _quick_fix(self):
        ...
        if not tab: return
        hi = tab["highlighter"]
        if not hi.lint_errors:
            self.st_left.configure(text="✓ No lint errors found"); return
        line_num, level, msg, start, end = hi.lint_errors[0]
        line_text = tab["text"].get(f"{line_num}.0", f"{line_num}.end")
        fixes = {
            r'\bvar\s+':  lambda l: re.sub(r'\bvar\s+', 'auto ', l),
            r'\blet\s+':  lambda l: re.sub(r'\blet\s+', 'auto ', l),
            r'\bif\s*\(': lambda l: re.sub(r'\bif\s*\((.+?)\)\s*(\{|$)', r'if \1 {', l),
        }
        for pattern, fix_fn in fixes.items():
            if re.search(pattern, line_text):
                new_line = fix_fn(line_text)
                tab["text"].delete(f"{line_num}.0", f"{line_num}.end")
                tab["text"].insert(f"{line_num}.0", new_line)
                hi.highlight()
                self.st_left.configure(text=f"✓ Quick Fix applied at line {line_num}")
                return
        messagebox.showinfo("Quick Fix", f"L{line_num}: {msg}\nAucune correction automatique disponible.")
    ...
    def _gen_doc(self):
        ...
        if not tab: return
        tw = tab["text"]
        line_num = int(tw.index("insert").split(".")[0])
        line = tw.get(f"{line_num}.0", f"{line_num}.end").strip()
        ...
        if m:
            ...
            doc  = f"// {'─' * 50}\n"
            doc += f"// @func   {fname}\n"
            doc += f"// @desc   TODO: describe {fname}\n"
            for p in param_list:
                doc += f"// @param  {p}  — TODO\n"
            doc += f"// @return TODO\n"
            doc += f"// {'─' * 50}\n"
            tw.insert(f"{line_num}.0", doc)
            tab["highlighter"].highlight()
            self.st_left.configure(text=f"✓ Doc generated for '{fname}'")
        else:
            messagebox.showinfo("Gen Doc", "Place cursor on a 'func ...' line first.")
    ...
    def _smart_import(self):
        ...
        if not tab: return
        code = tab["text"].get("1.0", "end-1c")
        existing = set(re.findall(r'^use\s+(\w+)', code, re.MULTILINE))
        needed = []
        checks = [
            ("Audio",    "use Audio(*)"),
            ("Turtle",   "use Turtle(*)"),
            ("hardware", "use hardware() as hard()"),
            ("System",   "use System(*)"),
            ("Math",     "use Math(*)"),
            ("Net",      "use Net(*)"),
            ("File",     "use File(*)"),
        ...
        for name, stmt in checks:
            if name in code and name not in existing:
                needed.append(stmt)
        if not needed:
            messagebox.showinfo("Smart Imports", "✓ All required imports are present.")
            return
        tab["text"].insert("1.0", "\n".join(needed) + "\n")
        tab["highlighter"].highlight()
        self.st_left.configure(text=f"✓ {len(needed)} import(s) added")

    # ── COLOR PICKER ─────────────────────────────────────────────────────────
    def _color_picker(self):
        from tkinter import colorchooser
        color = colorchooser.askcolor(title="Color Picker", parent=self)
        if color and color[1]:
            tw = self._active_text()
            if tw: tw.insert("insert", color[1])
            self.st_left.configure(text=f"🎨 Color inserted: {color[1]}")

    # ── SPLIT VIEW ───────────────────────────────────────────────────────────
    def _toggle_split(self):
        tab = self._active_tab_data()
        if not tab: return
        if not hasattr(self, "split_mode"): self.split_mode = False
        if self.split_mode:
            if hasattr(self, "_split_pane"):
                self._split_pane.pack_forget()
                del self._split_pane
            self.split_mode = False
            self.st_left.configure(text="Split view closed")
        else:
            sep = tk.Frame(self.editor_area, bg=t("border"), width=2)
            sep.pack(side="left", fill="y")
            pane = tk.Frame(self.editor_area, bg=t("bg"), width=380)
            pane.pack(side="left", fill="y")
            pane.pack_propagate(False)
            code = tab["text"].get("1.0", "end-1c")
            st = tk.Text(pane, bg=t("bg2"), fg=t("fg"), font=F_CODE, borderwidth=0,
                         highlightthickness=0, state="disabled", wrap="none", padx=8, pady=4)
            st.pack(fill="both", expand=True)
            st.configure(state="normal"); st.insert("1.0", code); st.configure(state="disabled")
            self._split_pane = pane
            self.split_mode = True
            self.st_left.configure(text="Split view — read-only copy  (Ctrl+\\ to close)")
    ...
    def _show_history(self):
        ...
        if not tab: return
        path = tab.get("path")
        # We build history from auto-saves during the session
        if not hasattr(self, "_file_history"): self._file_history = {}
        history = self._file_history.get(path, [])
        if not history:
            messagebox.showinfo("History", "No snapshots yet.\nSave the file to create history snapshots."); return
        w = tk.Toplevel(self); w.title("File History"); w.configure(bg=t("bg")); w.geometry("600x380")
        tk.Label(w, text=f"🕐  File History: {tab['name']}", bg=t("bg"), fg=t("accent"), font=F_TITLE).pack(pady=8)
        lb = tk.Listbox(w, bg=t("bg2"), fg=t("fg"), font=F_UI, borderwidth=0, highlightthickness=0)
        lb.pack(fill="both", expand=True, padx=12, pady=4)
        for ts, _ in reversed(history): lb.insert("end", f"  🕐  {ts}")
        def restore():
            sel = lb.curselection()
            if not sel: return
            idx = len(history) - 1 - sel[0]; _, content = history[idx]
            tab["text"].delete("1.0", "end"); tab["text"].insert("1.0", content)
            tab["highlighter"].highlight(); w.destroy()
            self.st_left.configure(text="✓ Snapshot restored")
        tk.Button(w, text="Restore Selected", command=restore,
                  bg=t("accent"), fg=t("bg"), relief="flat", font=F_UIB, padx=16).pack(pady=8)

    # ── SESSION ──────────────────────────────────────────────────────────────
    def _save_session(self):
        if not hasattr(self, "_session_path"):
            self._session_path = str(Path.home() / ".rint_studio_session.json")
        session = {"tabs": [], "active": self.active_tab, "theme": _T, "cwd": self.term_cwd}
        for tab in self.tabs:
            if tab.get("path"):
                session["tabs"].append({"path": tab["path"], "name": tab["name"]})
        with open(self._session_path, "w", encoding="utf-8") as f:
            json.dump(session, f, indent=2)
        self.st_left.configure(text=f"💼 Session saved  ({len(session['tabs'])} files)")
    ...
    def _load_session(self):
        ...
        try:
            ...
            if "theme" in session: self._set_theme(session["theme"])
            if "cwd" in session: self.term_cwd = session["cwd"]; self._refresh_tree()
            for tab_info in session.get("tabs", []):
                fpath = tab_info.get("path")
                if fpath and Path(fpath).exists():
                    with open(fpath, encoding="utf-8", errors="replace") as f: content = f.read()
                    idx = self.new_tab(content, tab_info.get("name", Path(fpath).name))
                    self.tabs[idx]["path"] = fpath
            self.st_left.configure(text="💼 Session loaded")
        except Exception as e:
            messagebox.showerror("Session", f"Error loading session: {e}")

    # ── MARKDOWN PREVIEW ─────────────────────────────────────────────────────
    def _md_preview(self):
        tab = self._active_tab_data()
        if not tab: return
        md = tab["text"].get("1.0", "end-1c")
        html = md
...
<style>body{{background:{t('bg')};color:{t('fg')};font:16px/1.6 sans-serif;max-width:820px;margin:40px auto;padding:0 24px}}
h1,h2,h3{{color:{t('accent')}}}code{{background:{t('bg3')};padding:2px 6px;border-radius:4px;font-family:monospace}}
pre{{background:{t('bg2')};padding:16px;border-radius:8px;overflow-x:auto;font-family:monospace}}
...
<body>{html}</body></html>"""
        ...
        self.st_left.configure(text="📄 Markdown preview opened")

    # ── AUTOSAVE LOOP ────────────────────────────────────────────────────────
    def _autosave_loop(self):
        interval = 0
        try:
            interval = int(self.ws_config.get("auto_save", "0"))
        except: pass
        if interval > 0:
            for tab in self.tabs:
                if tab.get("path") and tab.get("modified"):
                    content = tab["text"].get("1.0", "end-1c")
                    with open(tab["path"], "w", encoding="utf-8") as f: f.write(content)
                    tab["modified"] = False; tab["dot"].configure(text="")
                    # Store snapshot in history
                    if not hasattr(self, "_file_history"): self._file_history = {}
                    path = tab["path"]
                    if path not in self._file_history: self._file_history[path] = []
                    self._file_history[path].append((datetime.now().strftime("%H:%M:%S"), content))
                    if len(self._file_history[path]) > 20: self._file_history[path].pop(0)
        self.after(max(5000, interval * 1000) if interval > 0 else 30000, self._autosave_loop)
    ...
    def _show_about(self):
        ...
        tk.Label(w, text="v6.4  ·  Professional Edition", bg=t("bg"),
